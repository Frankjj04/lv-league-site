...
/* Decodes the data URL the browser produced, and checks it really is a JPEG or
   ...
   Returns { buf, type } or { code, error }, where code is 'bad' (did not
   arrive intact), 'big' or 'type' (not an image). Callers prefix the code
   with which photo it was. */
export function decodePhoto(dataUrl) {
  const m = /^data:(image\/(?:jpeg|png));base64,([A-Za-z0-9+/=]+)$/.exec(dataUrl || '');
  if (!m) return { code: 'bad', error: 'La foto no llegó correctamente.' };

  let buf;
  try { buf = Buffer.from(m[2], 'base64'); }
  catch { return { code: 'bad', error: 'La foto no llegó correctamente.' }; }

  if (!buf.length) return { code: 'bad', error: 'La foto no llegó correctamente.' };
  if (buf.length > MAX_PHOTO_BYTES) return { code: 'big', error: 'La foto pesa demasiado.' };

  const jpeg = buf[0] === 0xFF && buf[1] === 0xD8;
  const png  = buf[0] === 0x89 && buf[1] === 0x50 && buf[2] === 0x4E && buf[3] === 0x47;
  if (!jpeg && !png) return { code: 'type', error: 'Ese archivo no es una imagen.' };
...
/* Normalises and checks everything except the photo and the waiver, which the
   ...
   Returns { player } or { error, code }. The code names the problem so the
   sign-up form can show it in the player's language, on the right field. */
export function validatePlayer(body, { requireGuardian = true } = {}) {
  ...
  if (!p.division) return fail('division', 'Elige la división.');
  if (!p.team)     return fail('team', 'Elige el equipo.');
  if (!p.name)     return fail('name', 'Escribe el nombre completo.');
  if (!p.address)  return fail('address', 'Escribe la dirección.');
  if (p.phone.length !== 10) return fail('phone', 'El teléfono debe tener 10 dígitos.');
  if (!/^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(p.email)) return fail('email', 'Ese email no parece correcto.');

  if (!/^\d{4}-\d{2}-\d{2}$/.test(p.dob)) return fail('dob', 'Pon la fecha de nacimiento.');
  const age = ageOn(p.dob);
  if (age === null || age < 4 || age > 100) return fail('dob_bad', 'Esa fecha no parece correcta.');

  p.isMinor = age < MINOR_AGE;

  if (p.isMinor) {
    if (requireGuardian) {
      if (!p.guardianName) return fail('guardian_name', 'Escribe el nombre del tutor.');
      if (p.guardianPhone.length !== 10) return fail('guardian_phone', 'El teléfono del tutor debe tener 10 dígitos.');
    }
  } else {
